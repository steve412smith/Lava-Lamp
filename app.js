...
const VERSION = 'v2.2 slow-controls test';
...
function particleTarget() { const d = currentSettings().density; return Math.round(8 + ease(d) * 58); }
...
function makeParticle(i) {
  ...
  return {
    ...
    vx: rand(-0.006,0.006), vy: rand(-0.006,0.006),
    r: rand(26, 74) * (1.1 + calm * 0.55) * densityBias,
...
function update(dtMs) {
  ...
  const energy = 0.0025 + Math.pow(s.energy, 2.4) * 0.085;
  ...
  for (const p of particles) {
    ...
    const maxV = 0.025 + Math.pow(s.energy, 1.7) * 0.62;
...
function drawBlob(p, glow, layer) {
  ...
  const glowScale = layer === 'soft' ? (1.05 + glow * 0.55) : (0.72 + glow * 0.25);
  const alphaScale = layer === 'soft' ? 0.25 : 0.18;
  const g = ctx.createRadialGradient(p.x, p.y, 0, p.x, p.y, r*2.05*glowScale);
  g.addColorStop(0, hexAlpha(p.color, 0.24 * alphaScale + glow * 0.025));
  g.addColorStop(.28, hexAlpha(p.color, 0.16 * alphaScale + glow * 0.018));
  g.addColorStop(.70, hexAlpha(p.color, 0.045 * alphaScale + glow * 0.007));
...
function render(now) {
  ...
  drawBackground(p);
  if (!paused) update(dt);
  const glow = Math.pow(s.glow, 2.2) * 0.28; // intentionally capped; no whiteout

  ctx.globalCompositeOperation = 'source-over';
  ctx.filter = `blur(${0.7 + glow * 4.0}px)`;
  particles.forEach(part => drawBlob(part, glow, 'soft'));

  // Keep highlights subtle; no additive full-screen burn.
  ctx.globalCompositeOperation = 'screen';
  ctx.filter = `blur(${glow * 1.4}px)`;
...
function randomize() {
  ...
  ui.energy.value = Math.floor(rand(0, 35));
  ui.density.value = Math.floor(rand(0, 42));
  ui.glow.value = Math.floor(rand(0, 45));
...
if ('serviceWorker' in navigator) window.addEventListener('load', () => navigator.serviceWorker.register('service-worker.js?v=2.2.0'));
